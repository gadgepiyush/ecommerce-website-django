...
def searchMatch(query, item):
    #return true only if id matches the item
...
def search(request):
    ...
    if len(allProds) == 0 or len(query) < 3:
        params = {'msg': "Item not found please enter a valid Query"}
...
def checkout(request):
    if request.method == "POST":
        ...
        # Request paytm to accept the amount for the transaction
        param_dict = {
...
def handlerequest(request):
    ...
    if verify:
        if response_dict['RESPCODE'] == '01':
            ...
            print('order was not sucessful' + response_dict['RESPMSG'])
